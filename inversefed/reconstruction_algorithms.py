...
def loss_steps_grads(model, inputs, labels, loss_fn=torch.nn.CrossEntropyLoss(), lr=1e-4, local_steps=4, use_updates=True, batch_size=0):
    ...
    for i in range(local_steps):
        ...
        gradients = OrderedDict((name, grad_part)
                                for ((name, param), grad_part)
                                in zip(patched_model.parameters.items(), grad))
    if use_updates:
    ...
    return list(patched_model.parameters.values()), list(gradients.values())
